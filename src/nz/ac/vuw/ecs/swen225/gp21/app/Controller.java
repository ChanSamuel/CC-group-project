...
/**
 * A Controller interface which provides implemented methods for interacting
 * with the game. Subclasses are expected to use this class by calling the
 * requisite methods to interact with the game.
 * 
 * <p>This class' exposed methods do not do anything immediately, but rather queue
...
public abstract class Controller {
  ...
  /**
   * Contains a queue of the actions issued by the Controller. This queue is
   * polled every tick.
   */
  ...
  /**
   * The level number. 0 For the test level, 1 for level 1, 2 for level 2.
   */
  ...
  /**
   * The game loop where all other modules are updated periodically. It is Package
   * private so that only Action can access it.
   */
  GameLoop gameLoop;

  /**
   * Constructs the Controller by initialising this object's fields and then
  ...
  /**
   * Informs the user of something. Does not necesarrily bring up a dialog.
  ...
  /**
   * The method invoked by world when chap enters the exit tile. 
  ...
  /**
   * The method invoked by world when Chap leaves the info tile.
   */
  ...
  /**
   * The method invoked by the world when Chap loses.
   */
  protected abstract void playerLost();

  /**
   * The method invoked by the world when the player gains an item.

   * @param playerInventory : the players inventory as a List
   * @param item : the gained item
   */
  protected abstract void playerGainedItem(List<Item> playerInventory, Item item);
  
  /**
   * The method invoked by the world when the player consumes an item.

   * @param playerInventory : the player's inventory as a List.
   * @param item : the item consumed
   */
  protected abstract void playerConsumedItem(List<Item> playerInventory, Item item);

  /**
   * The method invoked by the world when Chap opens a door.
   */
  protected abstract void openedDoor();

  /**
   * The method invoked by the world when Chap collects a chip.

   * @param remainingChips : the chips yet to be collected.
   */
  protected abstract void collectedChip(int remainingChips);

  /**
   * The method invoked by the world when something gets teleported.
   */
  protected abstract void objectTeleported();

  /**
   * The method invoked by the world when something gets pushed.
   */
  ...
  /**
   * The method invoked by the game loop when the timer must be updated.
   * When invoked by the game loop, this method should display the new 
   * time with the new updated given time.

   * @param timeLeft : the time left to play the game.
   */
  protected abstract void updateTimerOperation(int timeLeft);

  /**
   * The method invoked by the game loop when we must pause the timer.
   */
  protected abstract void pauseOperation();

  /**
   * Convienent method to clear the player's inventory.
   */
  void clearInventory() {
  ...
  /*
   * ********************************************* 
   * METHODS FOR ACTIONS THE
   * CONTROLLER CAN ISSUE. 
   * *********************************************
   */

  /**
   * Issue an Action to the game by adding it to the Action queue. The Action may
   * not be proccessed immediately. Actions can be moves, or requests to pause the
   * game.
  ...
  /**
   * Attempts to move Chap up. The move may not be attempted immediately.
   */
  public void moveUp() {
  ...
  /**
   * Attempts to move Chap down. The move may not be attempted immediately.
   */
  ...
  /**
   * Attempts to move Chap left. The move may not be attempted immediately.
   */
  ...
  /**
   * Attempts to move Chap right. The move may not be attempted immediately.
   */
  ...
  /**
   * Exits the game without saving. This method performs a hard System.exit(...)
   * which will close all threads upon invocation.
   */
  public void exit() {
    System.exit(0);
  }
  ...
  /**
   * Exits the game and saves the latest state into a file. The game can be
   * resumed later on.
  ...
  /**
   * Loads a file to be replayed. After calling this method, the game should be in
   * a 'replay' state.
  ...
  /**
   * Pauses the game loop and sets the Controller to refuse all Actions other than
   * a resume. Resumes the game loop if already paused.
   */
   ...
   * @param f : Load a game from an XML file.
   */
  ...
  /**
   * Save the current game state to a file. Warns the user if a game isn't being
   * currently played.

   * @param saveFile : the XML file to save to.
   */
  ...
  /**
   * Save the current game into a replay file. Warns the user if a game isn't
  ...
  /**
   * Change the speed that the replay is being played at. Any values between 
   * [0.2, 5] inclusive are acceptable speeds. Warns the user if the speed is not
   * acceptable. Warns the user if the value isn't a number.

   * @param val : speed as a number.
   */
  ...
  /**
   * Change the speed that the replay is being played at. Any values between [0.2,5] 
   * inclusive are acceptable speeds. Warns the user if the speed is not
   * acceptable.

   * @param multiple : the coefficent to multiply the playback speed by.
   */
  ...
  /**
   * Go back a tick during a replay. Warns the user if the game isn't in replay
   * mode yet.
   */
  ...
  /**
   * Go forward a tick during a replay. Warns the user if the game isn't in replay
   * mode yet.
   */
  ...
  /**
   * Exits to main menu. For FuzzController, this will simply move into the menu
   * state, where only loading actions can be performed.
   */
